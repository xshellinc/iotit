...
func (rc *ROMClient) connect() error {
	...
	for i := 1; i <= numConnectAttempts; i++ {
		...
		if err == nil {
			...
			log.Warningf("Sync #%d failed: %s", i, err)
		}
	}
...
func (rc *ROMClient) sendCommand(cmd romCmd, arg []byte, csum uint8) error {
	log.Debugf("=> {cmd:%s csum:%d arg(%d):%q}", cmd, csum, len(arg), common.LimitStr(arg, 32))
...
func (rc *ROMClient) recvResponse() (*romResponse, error) {
	...
	log.Debugf("<= {cmd:%s value:%d ok:%t lastError:%d body(%d):%q}", r.cmd, r.value, r.ok, r.lastError, len(r.body), common.LimitStr(r.body, 32))
	return r, nil
}
...
func (rc *ROMClient) MemWrite(data []byte, addr, jumpAddr uint32) error {
	...
	log.Debugf("memWrite(0x%08x, %d, 0x%08x)", addr, len(data), jumpAddr)
...
func (rc *ROMClient) RunStub(stubJSON []byte, params []uint32) error {
	...
	log.Debugf("RunStub: params: %d @ 0x%08x; code: %d @ 0x%08x, entry @ %08x; data: %d @ 0x%08x",
...
func (rc *ROMClient) ReadReg(reg uint32) (uint32, error) {
	...
	log.Debugf("ReadReg(0x%08x) => 0x%08x", reg, r.value)
	return r.value, nil
}
...
func (rc *ROMClient) WriteReg(reg, value uint32) error {
	...
	log.Debugf("WriteReg(0x%08x, 0x%08x)", reg, value)
	return nil
}
