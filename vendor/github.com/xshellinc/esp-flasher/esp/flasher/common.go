...
func ConnectToFlasherClient(ct esp.ChipType, opts *esp.FlashOpts) (*cfResult, error) {
	...
	if err != nil {
		return nil, errors.Annotatef(
			err,
			"Failed to talk to bootloader. You "+
				"might need to put the device into flashing mode manually by "+
...
func detectFlashSize(fc *FlasherClient) (int, int, error) {
	...
	sizeExp := (chipID & 0xff)
	log.Debugf("Flash chip ID: 0x%08x, mfg: 0x%02x, sizeExp: %d", chipID, mfg, sizeExp)
