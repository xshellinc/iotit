...
import (
	...
	"github.com/xshellinc/esp-flasher/esp8266"
...
func (fc *FlasherClient) connect(baudRate uint) error {
	...
	if baudRate > 0 {
		log.Debugf("Switching to %d...", baudRate)
...
func (fc *FlasherClient) recvResponse() ([][]byte, error) {
	...
	for {
		...
		log.Debugf("<= %q", hex.EncodeToString(buf[:n]))
...
func (fc *FlasherClient) Write(addr uint32, data []byte, erase bool) error {
	...
	for numWritten < uint32(len(data)) {
		...
		log.Debugf("<= %d %d; %d", numWritten, bufLevel, numSent-numWritten)
		for numSent < uint32(len(data)) {
			...
			numSent += uint32(ns)
			log.Debugf("=> %d; %d/%d/%d", ns, numWritten, numSent, len(data))
		}
	}
	...
	log.Debugf("Write stats: waitTime:%.2f writeTime:%.2f eraseTime:%.2f miscTime:%.2f totalTime:%d",
...
func (fc *FlasherClient) Read(addr uint32, data []byte) error {
	...
	for numRead < len(data) {
		...
		log.Debugf("<= %d; %d/%d", len(buf), numRead, len(data))
	}
	tail, err := fc.recvResponse()
	if err != nil {
