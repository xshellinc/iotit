...
func adjustSysParamsLocation(fw *common.FirmwareBundle, flashSize int) {
	...
	for _, p := range fw.Parts {
		...
		if p.ESPFlashAddress != sysParamsAddr {
			log.Infof("Sys params image moved from 0x%x to 0x%x", p.ESPFlashAddress, sysParamsAddr)
...
func dedupImages(fc *FlasherClient, images []*image) ([]*image, error) {
	...
	for _, im := range images {
		log.Debugf("%d @ 0x%x", len(im.data), im.addr)
		imAddr := int(im.addr)
		...
		for offset < len(im.data) {
			...
			expectedDigestHex := strings.ToLower(hex.EncodeToString(expectedDigest[:]))
			log.Debugf("0x%06x %4d %s %s %t", imAddr+offset, blockLen, expectedDigestHex, digestHex, expectedDigestHex == digestHex)
			if expectedDigestHex == digestHex {
				...
				if newLen > 0 {
					...
					log.Debugf("%d @ 0x%x", len(nim.data), nim.addr)
		...
		if newLen > 0 {
			nim := &image{addr: uint32(newAddr), data: im.data[newAddr-imAddr : newAddr-imAddr+newLen], part: im.part}
			newImages = append(newImages, nim)
			log.Debugf("%d @ %x", len(nim.data), nim.addr)
			newTotalLen += newLen
			newAddr, newLen = 0, 0
		}
		log.Debugf("%d @ 0x%x -> %d", len(im.data), im.addr, newTotalLen)
