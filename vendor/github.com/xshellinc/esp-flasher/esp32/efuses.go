...
func (b *FuseBlock) Read() error {
	log.Debugf("Reading eFuse block %d @ 0x%08x...", b.num, b.rBase)
...
func (b *FuseBlock) WriteDiffs() error {
	log.Debugf("Writing eFuse block %d @ 0x%08x...", b.num, b.wBase)
...
func (f *Fuse) IsReadable() bool {
	...
	if err != nil {
		// Must not happen
		log.Fatalf("could not get rd_disable flag value")
	}
...
func (f *Fuse) IsWritable() bool {
	...
	if err != nil {
		// Must not happen
		log.Fatalf("could not get wr_disable flag value")
	}
...
func reverseKey(kb []byte) {
	if len(kb) != KeyLen {
		log.Fatalf("want %d bytes, got %d", KeyLen, len(kb))
	}
	for wi := 0; wi < 4; wi++ {
		for bi := 0; bi < 4; bi++ {
			i := 4*wi + bi
